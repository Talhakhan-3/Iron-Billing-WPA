    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={handleSync} className="p-2 text-slate-400 hover:text-teal-600 rounded-lg hover:bg-teal-50 transition-colors">
            <RefreshCw size={18} />
          </button>
          <button
            onClick={() => navigate('new-bill')}
            className="flex items-center gap-1.5 bg-teal-600 text-white px-4 py-2 rounded-xl font-medium text-sm hover:bg-teal-700 transition-colors shadow-sm"
          >
            ...
            return (
              <div key={day.date} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full flex flex-col justify-end" style={{ height: '80px' }}>
                  <div
                    className={`w-full rounded-t-lg transition-all ${isToday ? 'bg-teal-500' : 'bg-slate-200'}`}
                    style={{ height: `${Math.max(pct, 4)}%` }}
                    title={fmt(day.revenue)}
                  />
                </div>
                <div className={`text-xs font-medium ${isToday ? 'text-teal-600' : 'text-slate-400'}`}>{day.label}</div>
              </div>
            )
          })}
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate('history')} className="text-xs text-teal-600 hover:underline">View all</button>
          </div>
                ...
                <div key={p.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm font-semibold text-teal-600 ml-2 flex-shrink-0">{fmt(p.revenue)}</span>
                    </div>
