export async function generateBillPDF(bill, settings = {}) {
  ...
  const adjustTotal    = adjustments.reduce((s, a) => s + (parseFloat(a.amount) || 0), 0)
  const freightAmt     = Number(bill.total) - adjustTotal
  ...
  // QR generate karo (after balance is calculated)
  let qrDataUrl = null
  if (upiId) {
    const { generateUPIQR } = await import('./generateQR.js')
    qrDataUrl = await generateUPIQR({
      upiId,
      name:       biz,
      amount:     balance > 0 ? balance : Number(bill.total),
      billNumber: bill.billNumber,
      note:       'Shree Transport Bill',
    })
  }
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* SIG */
  .sig-qr-row{display:grid;grid-template-columns:1fr auto 1fr;border-bottom:1px solid #999;}
  .qr-box{padding:8px 12px;border-left:1px solid #ddd;border-right:1px solid #ddd;text-align:center;}
  .sb{padding:12px 18px;}
  .sb.sr2{text-align:right;}
  .sig-note{font-size:8.5px;color:#555;margin-bottom:4px;}
  .sig-line{border-bottom:1px solid #111;margin-top:28px;}
  .sig-cap{font-size:8px;color:#555;margin-top:3px;text-transform:uppercase;letter-spacing:0.07em;}
...
<div class="invoice">
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td class="r">${fmt(freightAmt)}</td>
        </tr>
        ${adjustments.length > 0 ? adjustments.map((adj, i) => `
        <tr>
          <td class="sno c">${i + 2}</td>
          <td class="desc" style="font-style:italic;color:#555;">${adj.label || 'Extra Charge'}</td>
          <td class="c">—</td>
          <td class="c">—</td>
          <td class="c">—</td>
          <td class="r">—</td>
          <td class="r" style="color:${parseFloat(adj.amount) < 0 ? '#b91c1c' : '#111'}">${parseFloat(adj.amount) >= 0 ? '+' : ''}${fmt(adj.amount)}</td>
        </tr>`).join('') : `
  ...
  <div class="bot">
    ...
    <div class="sc">
      <div class="sr"><span class="lb">Sub Total (Freight)</span><span class="vl">${fmt(freightAmt)}</span></div>
      ${adjustments.length > 0 ? adjustments.map(adj => `
      <div class="sr"><span class="lb" style="font-style:italic;">${adj.label || 'Extra'}</span><span class="vl" style="color:${parseFloat(adj.amount) < 0 ? '#b91c1c' : '#111'}">${parseFloat(adj.amount) >= 0 ? '+' : ''}${fmt(adj.amount)}</span></div>`).join('') : ''}
  ...
  <div class="sig-qr-row">
    <div class="sb">
      <div class="sig-note">Consignor Acknowledgement</div>
      <div class="sig-line"></div>
      <div class="sig-cap">Receiver's Signature &amp; Stamp</div>
    </div>
    ${qrDataUrl ? `
    <div class="sb qr-box">
      <div class="sig-note">Scan to Pay (UPI)</div>
      <img src="${qrDataUrl}" style="width:80px;height:80px;display:block;margin:4px auto 0;" alt="UPI QR"/>
      <div style="font-size:7.5px;color:#555;text-align:center;margin-top:3px;">${upiId}</div>
      ${balance > 0
        ? `<div style="font-size:8px;font-weight:700;color:#b91c1c;text-align:center;">Due: ₹${Number(balance).toLocaleString('en-IN')}</div>`
        : `<div style="font-size:8px;color:#2d6a4f;text-align:center;font-weight:600;">Full Amount</div>`
      }
    </div>` : '<div class="sb"></div>'}
    <div class="sb sr2">
      <div class="sig-note">For ${biz}</div>
      ${signatureBase64
        ? `<div style="margin-top:6px;text-align:right;">
             <img src="${signatureBase64}" style="max-height:40px;max-width:140px;object-fit:contain;" alt="Signature"/>
           </div>
           <div style="border-bottom:1px solid #333;margin-top:2px;"></div>`
        : `<div class="sig-line"></div>`
      }
      <div class="sig-cap">Authorised Signatory</div>
    </div>
  </div>
